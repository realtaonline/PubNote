...
set "XSL=%THIS%\..\xsl\PubNoteIndent.xsl"
set "LOG=%FILEDIR%\%~nx1.extract.log.txt"

echo Extracting "%FILEDIR%\%INPUT%" to ""%FILEDIR%\%INPUTBASE%\"...

java -jar "%SAXON_JAR%" -s:"%FILEDIR%\%INPUT%" -xsl:"%XSL%" -o:"%FILEDIR%\%INPUTBASE%\%INPUTBASE%.indent.xml" 2> %LOG%
...
:usage
echo Usage: PubNoteInIndent.bat file.xml
exit /b 1
